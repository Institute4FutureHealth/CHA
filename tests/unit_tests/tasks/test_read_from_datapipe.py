...
def read_datapipe_task():
    return ReadDataPipe(datapipe=initialize_datapipe(datapipe=DatapipeType.MEMORY))


def test_execute(read_datapipe_task):
    data_to_store = {'data': 'test_data', 'description': 'test_description'}
    key = read_datapipe_task.datapipe.store(json.dumps(data_to_store))
    result = read_datapipe_task.execute(f"$datapipe:{key}$")
