import pytest
from orchestrator.orchestrator import Orchestrator, Action, PlanFinish
...
def test_response_generator_generate_prompt(sample_orchestrator):
    ...
    previous_actions = [Action("example_task", "example_input")]
    use_history = True
    result = sample_orchestrator.response_generator_generate_prompt(
        final_response, history, meta, previous_actions, use_history
    )
    assert isinstance(result, str)
...
def test_plan(sample_orchestrator):
    ...
    previous_actions = [Action("example_task", "example_input")]
    use_history = True
    result = sample_orchestrator.plan(query, history, meta, previous_actions, use_history)
...
def test_generate_final_answer(sample_orchestrator):
    ...
    result = sample_orchestrator.generate_final_answer(query, thinker)
    assert isinstance(result, str)
...
def test_run(sample_orchestrator):
    ...
    history = "example_history"
    use_history = True
    result, previous_actions = sample_orchestrator.run(query, meta, history, use_history)
    assert isinstance(result, str)
