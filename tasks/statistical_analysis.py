    ...
    **Description:**
    ...
    def _execute(
        ...
        try:
            ...
            return json.dumps({"Data": "No data for the selected date(s)!"})
        
        if df.empty:
            return json.dumps({"Data": "No data for the selected date(s)!"})
        
        analysis_type = inputs[1].strip()
        columns_to_include: Optional[List[str]] = inputs[2].split(',') if len(inputs) > 2 else None
    ...
    def _calculate_slope(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for column in df.columns:
            if column != "date":
                trend_df[column] = [self._linear_regression_slope(df["date"], df[column])]
        return trend_df

    def _linear_regression_slope(self, x: pd.Series, y: pd.Series) -> float:
        n = len(x)
        sum_x = x.sum()
        sum_y = y.sum()
