...
class PpgGet(Affect):
    ...
    description: str = ("Get the Photoplethysmogram (PPG) signal for a specific date or "
    ...
    inputs: List[str] = ["user ID in string. It can be refered as user, patient, individual, etc. Start with 'par_' following with a number (e.g., 'par_1').",
                         ...
                         "end time of the PPG signal in string with the following format: '%Y-%m-%d %H:%M:%S'. If there is no end date, the value should be an empty string (i.e., '')"]
    outputs: List[str] = ["Photoplethysmogram (PPG) is a biomedical signal collected via an optical technique that indicates synchronous blood volumetric changes in the microvascular bed of tissues. Variations in PPG signals are associated with the oscillation of heartbeat and respiration.",
    ...
    def _execute(
        ...
        df_ppg_chunks = self._split_dataframe_by_time(
            df=df, sampling_frequency=sampling_frequency, window_length=5)

        # Convert the output to json
        json_out = self._ppg_dataframes_and_fs_to_json(dataframes_list=df_ppg_chunks,
